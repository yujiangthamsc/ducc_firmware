...
#endif
    HAL_GPIO_Write(DTR_UC, 0);      // LOW on DTR disables low-power mode / wakes up modem
...
bool MDMParser::powerOn(const char* simpin)
{
    ...
    HAL_Pin_Mode(DTR_UC, OUTPUT); // THIS WAS HOOKED TO LVLOE_UC before v016 HARDWARE!!
    HAL_GPIO_Write(DTR_UC, 0);    // LOW on DTR disables low-power mode / wakes up modem,
    ...
    HAL_Pin_Mode(DTR_UC, OUTPUT); // THIS WAS HOOKED TO LVLOE_UC before v016 HARDWARE!!
    HAL_GPIO_Write(DTR_UC, 0);    // FOR NOW JUST SET LOW, WITHOUT LOW-POWER MODE ENABLED
...
bool MDMParser::init(DevStatus* status)
{
    ...
    if (_dev.lpm != LPM_DISABLED) {
        ...
        if (RESP_OK != waitFinalResp())
            goto failure;
        _dev.lpm = LPM_ACTIVE;
        HAL_GPIO_Write(DTR_UC, 1);    // LOW diables, HIGH allows low-power mode
    }
